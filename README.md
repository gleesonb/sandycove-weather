...
- **AI text forecast** — a natural language summary generated by Cloudflare Workers AI (Meta Llama 3.1 8B model), written in a conversational Irish weather style. The AI analyzes the hourly forecast data to produce a human-readable forecast with local context. Cached for 30 minutes to minimize AI API usage.

### Sea Conditions
Sea surface temperature, wave height, and wave period from the **Dublin Bay Buoy** — a live in-situ sensor in the bay (Irish Lights MetOcean data, via [dublinbaybuoy.com](https://dublinbaybuoy.com)). If the buoy goes offline or stale, sea data falls back to the Open-Meteo marine model, and the card labels which source is active. Dublin Port tide times (high and low) come from the Marine Institute Ireland ERDDAP API — today and tomorrow, up to 8 tide events.

### Time for a Swim?
A swim suitability indicator for the Forty Foot. One shared rubric (`src/lib/swim-score.ts`) scores five factors — wind (speed *and* direction), rain, sea temp, waves, and tide state — each worth 0–20, summed to a 0–100 verdict from "Perfect for a swim!" down to "Best to stay dry", with a breakdown of each factor. Tide is weighted heavily: the Forty Foot is best at high or slack-high water, so low tide scores near zero. Wind direction matters too — onshore winds off the bay are penalised. A "Best time for a swim today" card ranks the top 3 remaining daylight hours with the same rubric, so the tide genuinely decides rather than every calm dry hour tying.
...
```
  ...
  Browser ──── GET ──────►│  Astro SSR + React Islands        │
    ...
    │  Underground  │  │     Map      │  │  (forecast &     │
    │  (station)    │  │  (forecast)  │  │   rain fallback, │
    │               │  │              │  │   sea fallback)  │
    └──────────────┘  └──────────────┘  └──────────────────┘
                                │
                         ┌──────────────┐
...
| `/api/forecast/text` | 30 min | AI-generated text forecast, cached to reduce API calls |
...
| AI/LLM | [Cloudflare Workers AI](https://developers.cloudflare.com/workers-ai/) with Meta Llama 3.1 8B (`@cf/meta/llama-3.1-8b-instruct`) |
...
| [Cloudflare Workers AI](https://developers.cloudflare.com/workers-ai/) | AI-generated text forecast using Meta Llama 3.1 8B model (analyzes forecast data for natural language summary) | Primary |
| [Open-Meteo](https://open-meteo.com/) | Forecast fallback, rain alarm nowcast, historical fallback, sea-data fallback | Fallback + supplementary |
| [Met Eireann](https://www.met.ie/) | Weather warnings | Primary |
...
| [Dublin Bay Buoy](https://dublinbaybuoy.com/) | Live in-situ sea temperature, wave height & period (Irish Lights MetOcean data) — primary sea source | Primary |
...
The AI text forecast feature uses Cloudflare Workers AI with the Meta Llama 3.1 8B model (`@cf/meta/llama-3.1-8b-instruct`). The AI binding is configured in `wrangler.deploy.toml`:
...
**IMPORTANT:** To enable Workers AI, your Cloudflare API key must have Workers AI permissions. Update your Cloudflare API key in the Cloudflare dashboard to include the "Workers AI" permission.

### Environment Setup

1. **Local development:** Create a `.env` file with your API keys
2. **Production secrets:** Use `wrangler secret put <SECRET_NAME>` to securely store credentials:
   - `OPENWEATHER_API_KEY` — OpenWeatherMap API key for forecast data
   - `WEATHER_UNDERGROUND_KEY` — Weather Underground API key for station data
...
*Weather from local station IDNLAO16 · Forecast from OpenWeatherMap · Warnings from Met Éireann · Sea data from Dublin Bay Buoy & Marine Institute Ireland · Forecast & sea fallback from Open-Meteo*
